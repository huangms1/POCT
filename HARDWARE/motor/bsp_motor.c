...
/**
 * @brief 计算电机需要移动的步数和方向
 * @param step 目标位置
 * @param motor_num 电机编号
 * @param dir 传入的方向
 * @param out_step 输出的步数
 * @param out_dir 输出的方向
 */
static void calculate_steps_and_direction(uint32_t step, uint8_t motor_num, uint8_t dir, uint32_t *out_step, uint8_t *out_dir) {
    if (step >= Stepper[motor_num].position) {
        *out_step = step - Stepper[motor_num].position;
        *out_dir = (dir == CCW) ? CW : CCW;
    } else {
        *out_step = Stepper[motor_num].position - step;
        *out_dir = (dir == CCW) ? CCW : CW;
    }
}
