...
//			if(len>0)
//			{
//				recv(SOCK_TCPS,Recv_Buff,len);								              	/*接收来自Client的数据*/
////				Recv_Buff[len] = 0x00; 											            /*添加字符串结束符*/
////				printf("%s\r\n",Recv_Buff);
//				/* USART1 向 DMA发出TX请求 */
////				HAL_UART_Transmit_DMA(&Uart3Handle, (uint8_t *)Recv_Buff ,len);
//				send(SOCK_TCPS,Recv_Buff,len);									            /*向Client发送数据*/
//		  }
//		break;
//		
...
//			if(len>0)
//			{
//				recv(SOCK_TCPC,Recv_Buff,len); 							   		     /*接收来自Server的数据*/
//				memset(Rx_DMA,0,len);/* 清零 */
...
void NetRecv(uchar * pData, ushort * usSize)
{
	...
	* usSize = getSn_RX_RSR(SOCK_TCPC);							/*定义len为已接收数据的长度*/
	
	if(* usSize>0)
	{
		recv(SOCK_TCPC,pData,* usSize); 						/*接收来自Server的数据*/
	}	
}
