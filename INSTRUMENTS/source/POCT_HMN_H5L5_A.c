...
uint32_t Fa_Count(uint8_t FaNum)
{
	...
	if((Globle.FaNum == FaNum)&&(Globle.FaNum!=0))
	{
		return 0;
	}
...
uint8_t Fa_Function(uint8_t FaNum)
{
	...
	{	
		...
		fre = Motor_Fre(Globle.POCT_Par.M09_P[0]);

		MotorFre_Abs_Run(Globle.POCT_Par.M09_P[0],M09,M09_DIR,fre);
	...
	else
	{
		...
		{ 
			...
			fre = Motor_Fre(Globle.POCT_Par.M09_P[1]);
			
			MotorFre_Abs_Run(Globle.POCT_Par.M09_P[1],M09,M09_DIR,fre);
...
{	
	...
	{		
		...
		if(M04_DIR == CW)
			Stepper_Move_S(50,DOOR_FRE,0.01f, 0xfffff,M04,CW,pSpeed);
		else
			Stepper_Move_S(50,DOOR_FRE,0.01f, 0xfffff,M04,CCW,pSpeed);
	...
	else 
	{
		...
		if(M04_DIR == CW)
			Stepper_Move_S(50,DOOR_FRE,0.01f, 4192*100,M04,CCW,pSpeed);
		else
			Stepper_Move_S(50,DOOR_FRE,0.01f, 4192*100,M04,CW,pSpeed);
