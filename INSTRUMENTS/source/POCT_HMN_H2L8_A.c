...
{
	uint32_t offset1 = 0,offset2 = 0,step;
...
uint8_t Fa_Function(uint8_t FaNum)
{
	...
	{	
		...
		MotorFre_Abs_Run(Globle.POCT_Par.M11_P1,M11,M11_DIR,400);
	}
	else
	{
		MotorFre_Abs_Run(Globle.POCT_Par.M11_P1,M11,M11_DIR,400);
...
//	if(num == 5)
//		LED_SPI_SendData(num ,(uint8_t)Globle.POCT_Par.led_cy7);
//		else
...
{	
	...
	{
		scan_module_reset();
//		Motor_Rst(M08_FRE,M08,M08_DIR,M08_GO_GPIO_PORT,M08_GO_GPIO_PIN,0);
//		Motor_Rst(M03_FRE,M03,M03_DIR,M03_GO_GPIO_PORT,M03_GO_GPIO_PIN,0);
	}
	else
	{
...
bool App_NZL(SMsg * pMsg,int * pFaultCode)
{
	...
	xEventGroupSetBits(ledEventGroupHandle,PCR_LED_FLAG); //指示灯
...
//	Fa_Function(DRYING);
	...
	PUMP_Func(5000,PUMP_IN,700);
	
	Fa_Function(EFFLUENT_1);
	
	PUMP_Func(23000,PUMP_OUT,700);
...
//	PUMP_Func(15000,PUMP_IN,600);	
	...
	PUMP_Func(32000,PUMP_IN,600);
	                                                                                         
	Fa_Function(WASHING_2);  
	
 	PUMP_Func(32000,PUMP_OUT,600); 
///////////////////////////////////	
//	Fa_Function(PUBLIC_HOLE);
//	
//	PUMP_Func(30000,PUMP_IN,700);	
...
//	temp_V[0] = Globle.POCT_Par.Liquid_v[0];
//	temp_V[1] = Globle.POCT_Par.Liquid_v[1];
	...
	PUMP_Mix_Func(5,15000,1);
	
	PUMP_Func(15000,PUMP_IN,500);

	Wait_Time(1);
	
	PUMP_Func(1900,PUMP_OUT,500); 
	...
	while(1)
	{
		...
		if (pKey != NULL)
		{
			...
			if (!strcmp(pValue,"1"))
			{
				...
				if(iIndex == 1)
				{
					Fa_NegativeDirection(9);
				}
				else if(iIndex == 2)
					Fa_NegativeDirection(11);
				
//				Fa_To_Hole(iIndex + 1);
				
				PUMP_Func(Globle.POCT_Par.Liquid_v[iIndex - 1],PUMP_OUT,500); 
...
void ALL_Motor_RESET(void)	
{
	...
	Err = Motor_Rst(M03_FRE,M03,M03_DIR,M03_GO_GPIO_PORT,M03_GO_GPIO_PIN,0);
...
{
	MotorFre_Abs_Run(Globle.POCT_Par.M05_P1,M05,M05_DIR,500);
}
