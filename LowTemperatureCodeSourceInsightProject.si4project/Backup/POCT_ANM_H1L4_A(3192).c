...
//				step = Globle.POCT_Par.M10_Fa_P[0] + (offset2 - offset0) + offset1*(FaNum-1);
				step = Globle.POCT_Par.M10_Fa_P[0] + offset0 + offset2 + offset1*(FaNum-1);
			}
			else
			{
	...
	else
	{
		if(Globle.LastFaNum > Globle.FaNum)
			...
			step = (Globle.POCT_Par.M10_Fa_P[0] + offset2 + offset1*(Globle.FaNum - 1)) - (offset1*(Globle.FaNum - FaNum));
	}
