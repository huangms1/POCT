...
bool App_NZL(SMsg * pMsg,int * pFaultCode)
{
	...
	{		
		...
		if(temp_up_cnt >= 100)
		{
			if((SendT2T3buf[2] > Globle.TempControl.Temp_Ch2_Set - 5)&&(SendT2T3buf[2] < Globle.TempControl.Temp_Ch2_Set + 5))
			{
				SendT2T3buf[2] =  Globle.TempControl.Temp_Ch2_Set;
			}
			else
			{
				SendT2T3buf[2] = GetTemp(1); 
			}
			
			if((SendT2T3buf[0] > Globle.TempControl.Temp_Ch3_Set - 5)&&(SendT2T3buf[0] < Globle.TempControl.Temp_Ch3_Set + 5))
			{
				SendT2T3buf[0] =  Globle.TempControl.Temp_Ch3_Set;
			}
			else
			{
				SendT2T3buf[0] = GetTemp(2); 
			}
		...
		vTaskDelay(10);				
		if(SendT2T3buf[2] >= Globle.TempControl.Temp_Ch2_Set - 2)
		{
		    TimeNowCnt  = HAL_GetTick();
...
bool App_YBX(SMsg * pMsg,int * pFaultCode)
{
	...
	{		
		...
		delay_ms(10);				
		if(SendT2T3buf[0] >= Globle.TempControl.Temp_Ch3_Set - 2)
		{
		    TimeNowCnt  = HAL_GetTick();
			countdown = 0;	
			temp_up_cnt = 0;
...
void App_PCR(SMsg * pMsg)
{
	...
	{		
		...
		if(SendT2T3buf[0] >= Globle.TempControl.Temp_Ch3_Set - 10)
		{
			fre =  Motor_Fre(Globle.POCT_Par.M02_P1);
